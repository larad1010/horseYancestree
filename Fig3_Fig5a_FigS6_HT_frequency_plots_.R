...
#Data and plots for Figure 3, Figure 5a, and Supporting Figure S6
...
##### Local-Asian ####
As<- Crown %>% filter(Breed.group.detailed=="Local riding and lightdraft - Asia")
...
dataAR <- dataAR %>%
  mutate(`Breed.group.detailed` = "Arabian")

dataTB <- data_TB %>%
  mutate(`Breed.group.detailed` = "Thoroughbred")

dataCB <- dataCB %>%
  mutate(`Breed.group.detailed` = "Coldblood")

dataSP <- dataSP %>%
  mutate(`Breed.group.detailed` = "Spanish influenced")

dataAS <- dataAS %>%
  mutate(`Breed.group.detailed` = "Local Asia")%>%
  select(-Geographic.region)

head(dataAS)
dataAS2 <- dataAS[, c(1, 4, 2,3)]

All<- rbind(dataAR,dataTB,dataCB,dataSP,dataAS2)
sum(All$n)
...
PAll<- ggplot(All) + 
  ...
  scale_fill_manual(labels = c("Arabian","Thoroughbred","Coldblood","Spanish influenced","Local Asia"),                    
                    values = c("Arabian"="#3335FF","Thoroughbred"="#FF1616","Coldblood"="#598037","Spanish influenced"="#F76D02","Local Asia"="#A66C98"))+
  # Add text showing the value of each 100/75/50/25 lines
...
ggsave(plot =PAll , file = "sunburst_all.png", 
       bg = "transparent",path="/Users/radovicl/Desktop/horseYancestree/",
       width = 30, height = 30, units = "cm", dpi = 600)
...
ggsave(plot =PRIDING , file = "sunburst_Riding.png", path="/Users/radovicl/Desktop/horseYancestree/",
       bg = "transparent",
       width = 30, height = 30, units = "cm", dpi = 600)
